...
2. **Upload Files:**
   - `predict.py`
   - `module2_vlm_reasoner.py`
   - `requirements.txt`
   - `TECHNICAL_REPORT.md`
   - `README.md`
   - Checkpoint file (if < 5GB) OR download link

3. **Add Model Card:**
   Copy content from `README.md` to Hugging Face model card

4. **Submit Form:**
   - URL: https://forms.office.com/r/864ac0pUAC
   - Provide Hugging Face repo link
   - Complete all required fields

### Option B: Direct Upload

If repo is too large:
1. Upload checkpoint to Google Drive/Dropbox
2. Update `predict.py` to download checkpoint automatically
3. Submit code + download link
...
### Issue: VLM Not Loading

**Solution:**
System automatically falls back to heuristic analysis. To fix:
```bash
...
python predict.py \
  ...
  --checkpoint /full/path/to/checkpoint.ckpt
```
...
**Solution:**
```bash
# Use lightweight VLM + GPU
python predict.py --input_dir /test_images --lightweight --device cuda
